...
def format_submission_for_llm(submission: Dict[str, Any]) -> str:
    """
    Format submission data as clean, readable text for LLM consumption.
    The code field is already unescaped by Python's JSON parser, so we can
    format it directly as readable text.
    
    Args:
        submission: Submission data dictionary from LeetCode API
    ...
    if "question" in submission:
        ...
        lines.append(f"Question ID: {question.get('questionId', 'N/A')}")
        lines.append(f"Title: {question.get('title', 'N/A')}")
        lines.append(f"Title Slug: {question.get('titleSlug', 'N/A')}")
        lines.append("")
    
    # Add code (this is already unescaped by Python's JSON parsing)
    if "code" in submission and submission["code"]:
        lines.append("Code:")
        lines.append("-" * 60)
...
async def get_submission_details(submission_id: int, format: str = "json"):
    """
    ...
    Args:
        ...
        format: Output format - "json" (default) returns JSON, "llm" returns cleaned text for LLM consumption
    """
    if not LEETCODE_SESSION:
        raise HTTPException(
            status_code=401, 
    ...
    try:
       async with get_authenticated_client() as client:
            ...
            if response.status_code == 200:
                ...
                # Return LLM-formatted text if requested
                if format.lower() == "llm":
                    formatted_text = format_submission_for_llm(submission)
                    return PlainTextResponse(content=formatted_text, media_type="text/plain")
                
                # Return JSON (code is already unescaped by Python's JSON parser)
                return submission
